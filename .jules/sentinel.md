...
## 2024-04-14 - Preventing User Enumeration and Timing Attacks in Authentication
**Vulnerability:** The login endpoint (`src/app/api/login/route.ts`) leaked account existence information by returning different error messages and status codes for non-existent users ('Account does not exist', 404) versus incorrect passwords ('Invalid password', 401). Additionally, a timing attack was possible because the expensive `bcrypt.compare` operation was skipped if the user did not exist.
**Learning:** Returning specific error messages for authentication steps helps attackers enumerate valid usernames/emails. Furthermore, skipping hash comparison allows attackers to deduce user existence based on response time differences.
**Prevention:** Always return a unified error message (e.g., 'Invalid email or password') and consistent HTTP status code (e.g., 401) for authentication failures. Crucially, simulate hash processing time by executing a dummy `bcrypt.compare` with a properly formatted hash string when a user is not found to defend against timing attacks.
