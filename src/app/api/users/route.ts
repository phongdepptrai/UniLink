...
export async function GET(request: NextRequest) {
  try {
    ...
    const [users, total] = await Promise.all([
      ...
      // ⚡ Bolt: Use estimatedDocumentCount() instead of countDocuments({}) for O(1) performance as it uses collection metadata
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ⚡ Bolt: Use .exists() instead of .findOne() for existence checks to avoid transferring the full document
